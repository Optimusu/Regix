...
public class PatientControlUnitOfWork : IPatientControlUnitOfWork
{
    ...
    public async Task<ActionResponse<IEnumerable<PatientControl>>> GetAsync(PaginationDTO pagination, string username) => await _patientService.GetAsync(pagination, username);
    ...
    public async Task<ActionResponse<PatientControl>> AddAsync(PatientControl modelo, string username) => await _patientService.AddAsync(modelo, username);
