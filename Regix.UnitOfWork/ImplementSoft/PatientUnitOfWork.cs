...
public class PatientUnitOfWork : IPatientUnitOfWork
{
    ...
    public async Task<ActionResponse<IEnumerable<Patient>>> GetAsync(PaginationDTO pagination, string username) => await _patientService.GetAsync(pagination, username);
    ...
    public async Task<ActionResponse<Patient>> AddAsync(Patient modelo, string username) => await _patientService.AddAsync(modelo, username);
