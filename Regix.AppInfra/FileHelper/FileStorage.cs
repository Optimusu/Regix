...
public class FileStorage : IFileStorage
{
    ...
    //public async Task RemoveFileAsync(string path, string containerName)  Para manejo Publicos sin permisos
    //{
    ...
    public async Task<string> SaveFileAsync(byte[] content, string fileName, string containerName, string fileNameOriginal, string mimeTypeEnviado)
    {
        ...
        if (!validExts.Contains(ext))
            throw new InvalidOperationException("Solo se permiten archivos Word (.doc, .docx) o PDF (.pdf)");
        ...
        if (mimeEsperado?.Trim().ToLowerInvariant() != mimeTypeEnviado?.Trim().ToLowerInvariant())
            throw new InvalidOperationException($"El tipo MIME no coincide con la extensión del archivo. Esperado: {mimeEsperado}, recibido: {mimeTypeEnviado}");
