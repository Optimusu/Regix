...
public partial class IndexIndentidadGenero
{
    ...
    public List<IdentidadGenero>? IdentidadGeneros { get; set; }
    ...
    private async Task Cargar(int page = 1)
    {
        ...
        var responseHttp = await _repository.GetAsync<List<IdentidadGenero>>(url);
    ...
    private async Task ShowModalAsync(int id = 0, bool isEdit = false)
    {
        if (isEdit)
        {
            ...
            await _modalService.ShowAsync<EditIndentidadGenero>(parameters);
        }
        else
        {
            var parameters = new Dictionary<string, object>
            {
                { "Title", "Create Gender Identity"  }
            };
            await _modalService.ShowAsync<CreateIndentidadGenero>(parameters);
        }
    }
