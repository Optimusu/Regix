...
public partial class IndexVeteran
{
    ...
    public List<Veteran>? Veterans { get; set; }
    ...
    private async Task Cargar(int page = 1)
    {
        ...
        var responseHttp = await _repository.GetAsync<List<Veteran>>(url);
    ...
    private async Task ShowModalAsync(int id = 0, bool isEdit = false)
    {
        if (isEdit)
        {
            ...
            await _modalService.ShowAsync<EditVeteran>(parameters);
        }
        else
        {
            var parameters = new Dictionary<string, object>
            {
                { "Title", "Create Veterans"  }
            };
            await _modalService.ShowAsync<CreateVeteran>(parameters);
        }
    }
