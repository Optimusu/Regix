...
public partial class IndexRegular
{
    ...
    private const string baseUrl = "api/v1/regulars";
    public List<Regular>? Regulars { get; set; }
    ...
    private async Task Cargar(int page = 1)
    {
        ...
        var responseHttp = await _repository.GetAsync<List<Regular>>(url);
    ...
    private async Task ShowModalAsync(int id = 0, bool isEdit = false)
    {
        if (isEdit)
        {
            ...
            {
                { "Id", id },
                { "Title", "Edit Regular Period"  }
            };
            await _modalService.ShowAsync<EditRegular>(parameters);
        }
        else
        {
            var parameters = new Dictionary<string, object>
            {
                { "Title", "Create Regular Period"  }
            };
            await _modalService.ShowAsync<CreateRegular>(parameters);
        }
    }
