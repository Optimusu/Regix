...
[ApiVersion("1.0")]
[Route("api/v{version:apiVersion}/regpatient3s")]
[Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme, Roles = "Patient, Administrator")]
[ApiController]
public class Patient3sController : ControllerBase
{
    private readonly IPatient3UnitOfWork _unitOfWork;
    private readonly IStringLocalizer _localizer;

    public Patient3sController(IPatient3UnitOfWork unitOfWork, IStringLocalizer localizer)
    {
    ...
    [HttpPut]
    public async Task<IActionResult> PutAsync(Patient3 modelo)
    {
        try
        {
    ...
    [HttpPost]
    public async Task<IActionResult> PostAsync(Patient3 modelo)
    {
        try
        {
            ClaimsDTOs userClaimsInfo = User.GetEmailOrThrow(_localizer);
